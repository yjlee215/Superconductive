...
top_feature = st.checkbox("Choosing top features and visualize")
if top_feature:
    st.write("""
...
fig_corr = plt.figure(figsize=(30,30))
...
dropping = st.checkbox("Drop Column")
if dropping:
    col_names = st.text_input("Enter a column name for drop column, e.g. Name")
    col_names_ = col_names.split()
    if col_names:
        Train = drop_columns(Train, col_names_)
        st.dataframe(Train.head())
